...
use SimpleCalendar\plugin_deps\GuzzleHttp\Promise\FulfilledPromise;
use SimpleCalendar\plugin_deps\GuzzleHttp\Promise\PromiseInterface;
use SimpleCalendar\plugin_deps\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \SimpleCalendar\plugin_deps\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \SimpleCalendar\plugin_deps\GuzzleHttp\Psr7\Utils::streamFor($sink);
